...
@FeignClient(name="account-api", url="http://localhost:8081/accounts", configuration = FeignClient.class)
public interface AccountApiClient {
    ...
    // 계정 하나 조회
    @GetMapping("/{ids}")
    ResponseEntity<AccountDto> getAccountByIds(@PathVariable("ids") String ids);
